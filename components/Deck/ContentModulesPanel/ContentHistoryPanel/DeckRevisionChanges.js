...
class DeckRevisionChanges extends React.Component {
    ...
    render() {
        const changes = this.props.changes && this.props.changes.length ? this.props.changes.map((change, index) => {
            return (
            <ContentChangeItem selector={this.props.selector} allowUndoDelete={this.props.allowUndoDelete && index === 0} permissions={this.props.permissions} type='deck' change={change} key={index}/>
            );
