    ...
    render() {
        let list = Object.keys(this.props.items).map((key) => {
            ...
            if (value instanceof Array) {
                return (<DeckPropertyEditList key={key} onChange={this.handleChange}
                    fieldname={key} fieldcontent={value} />);
            } else {
                let strVal = String(value);
                if (strVal.length < 300)    // TODO find a good value (depend on screen mode?)
                    return (<DeckPropertyEditText  key={key} onChange={this.handleChange}
                        fieldname={key} fieldcontent={value} />);
                else
                    return (<DeckPropertyEditBigText  key={key} onChange={this.handleChange}
                        fieldname={key} fieldcontent={value} />);
            }
        });
