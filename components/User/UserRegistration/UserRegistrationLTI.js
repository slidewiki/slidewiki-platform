...
import ltiSignUp from '../../../actions/user/registration/ltiSignUp';
...
const MODI = 'ltilogin_modi';
const NAME = 'ltilogin_data';

class UserRegistrationLTI extends React.Component {
    constructor(props) {
        super(props);
        this.provider = '';
    ...
    componentDidMount() {
        ...
        $(ReactDOM.findDOMNode(this.refs.UserLTIRegistration_form)).form(validationRules);

    }

    componentWillReceiveProps(nextProps) {
        // console.log('UserRegistrationLTI componentWillReceiveProps()', this.props.UserRegistrationStore.LTIuserdata, nextProps.UserRegistrationStore.ltiuserdata);
        if (nextProps.UserRegistrationStore.ltiuserdata.email === undefined && nextProps.UserRegistrationStore.ltiuserdata.username === undefined) {
            this.setUserdata({}, false);
            return;
        }
        if (nextProps.UserRegistrationStore.ltiuserdata && localStorage.getItem(MODI) === 'register') {
    ...
    handleSignUp(e) {
        ...
        $(ReactDOM.findDOMNode(this.refs.LTIRegistration_Modal)).modal('hide');
        let user = this.props.UserRegistrationStore.ltiuserdata;
        ...
        this.context.executeAction(ltiSignUp, user);
        return false;
    }
    ...
    setUserdata(data, check = true) {
        // console.log('UserRegistrationLTI setUserdata()', data);
    ...
    render() {
        ...
        return (
          <div>
            <div className="ui ltiregistration modal" id='signinModal' style={modalStyle} ref="LTIRegistration_Modal" >
              <div className="header">
                  <h1 style={headerStyle}>Validate user information</h1>
              </div>
              <div className="content">
                  <form className="ui registrationmodalform form" ref="UserLTIRegistration_form" >
...
UserRegistrationLTI.contextTypes = {
    executeAction: React.PropTypes.func.isRequired
};
UserRegistrationLTI = connectToStores(UserRegistrationLTI, [UserRegistrationStore], (context, props) => {
    return {
        UserRegistrationStore: context.getStore(UserRegistrationStore).getState()
    };
});
export default UserRegistrationLTI;
