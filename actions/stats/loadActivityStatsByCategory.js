...
export default function loadUserStats(context, payload, done) {
    ...
    context.service.read('stats.userActivitiesByCategory', payload, {timeout: 20 * 1000}, (err, res) => {
        if (err) {
            log.error(context, {filepath: __filename});
            context.executeAction(serviceUnavailable, payload, done);
        } else {
            context.dispatch('LOAD_ACTIVITY_STATS_BY_CATEGORY', {activitiesByCategory: res});
        }
        done();
    });
}
